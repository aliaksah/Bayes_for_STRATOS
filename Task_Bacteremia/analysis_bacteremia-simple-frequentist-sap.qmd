...
This frequentist analysis follows the simplified statistical analysis plan by Michael Kammer dated 2.6.2025 (see version history). IDA and descriptives are presented in an [overview notebook](./analysis_overview-bacteremia.html).

## Histoy

As of May 2025 it was decided to simplify the analysis compared to the first versions of the SAP, and to remove variable selection from the task. We now use ridge regression instead.
...
Data were downloaded from [Zenodo](https://zenodo.org/records/7554815) and stored locally for ease of use.
...
#' Helper to compute performance metrics for logistic glmnet model
#'
#' @details
#' Uses `rms::val.prob` to compute most prediction metrics.
compute_performance_glmnet <- function(m, x, y) {
...
#' Helper to fit logistic ridge model
#' 
#' @return 
#' List with entries: 
#' 
#' - `m_ridge`: ridge glmnet object
#' - `perf`: apparent performance
#' - `scaler`: scaler object
fit_ridge <- function(d, f, lambda = NULL) {
    ...
    perf = compute_performance_glmnet(m_ridge, x, d$BloodCulture)
...
do_cv <- function(d, folds, f_m, lambda, seed = 678) {
    foreach(
        i = seq_along(folds), 
        .errorhandling = "pass", 
        ...
        res_t = fit_ridge(d_t, f_m, lambda = lambda)
        
        d_v %<>% 
            apply_scale(res_t$scaler) %>% 
            mutate(SEX = SEX - 1)
        x_v = model.matrix(f_m, d_v)[, -1]
        perf_v = compute_performance_glmnet(res_t$m_ridge, x_v, 
                                           d_v[[all.vars(f_m)[1]]])
...
do_bootstrap <- function(df, b_ind, variables,  
    ...
    for (i in 1:imp_m) {
        ...
        res_out$res_cv = do_cv(d_b_imp, res_out$folds, f_m,
                               res_out$m_ridge$lambda, seed = res_out$seed_inner)
        ...
        perf_oob = compute_performance_glmnet(res_out$m_ridge, x_oob, 
...
Imputation model is only re-fitted if no model file in `r params$folder_imputation` is found. Note that we used a parallel version of mice to speed up computations.
...
res = fit_ridge(d_imp, f_m)
```

Ridge coefficient paths.

```{r}
plot(res$m_ridge, xvar = "lambda")
```
...
res_cv = do_cv(d_imp, folds, f_m, res$m_ridge$lambda, seed = 1868)
plan(sequential)
```
...
glue("Best performance at lambda {opt_lambda} (log-lambda {log(opt_lambda)}):")
...
coef_m = coef(res$m_ridge, s = opt_lambda)
...
Final coefficients by averaging over imputations.

```{r rows.print=50}
coef_imp = map(res_imp, "m_ridge") %>% 
...
coef_final = coef_imp %>% 
    ...
    summarise(
        ...
        coefficient_cv = coefficient_sd / abs(coefficient_mean)
    )
...
file_bootstrap = file.path(params$folder_analysis, 
                           sprintf("bootstrap-ridge%s.rds", SUFFIX))
