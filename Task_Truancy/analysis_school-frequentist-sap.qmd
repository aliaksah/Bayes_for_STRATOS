...
do_bootstrap <- function(df, ind, f_b, VARIABLES) {
    ...
    list(
        preddf_bi = preddf_bi %>% select(bull, ATT4, pred_bi, quantile)
    )
}
```
...
- install `quarto`
- make sure the data is in the correct folder (`Data/Raw`)
- build using "Render" in RStudio, or using the quarto package.
