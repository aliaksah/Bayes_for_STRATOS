##### hold-out-PPC nur für binäre bei den beiden Modellen, bis Modell 3, dann LOO-CV und Stacking der drei Modelle, sens indem man das dritte Modell modifiziert, Ergebnisse wie bisher
...
br.candL20 <- list()
for (i in 1:n01.models) {
  br.candL20[[i]] <- brierS(pisa18$attp[pisa18$WITHOUT == 0],colMeans(ypredCand0[[i]]))
...
### Posterior predictive checks, use hold-out predictors
```{r  eval = FALSE}

color_scheme_set(scheme = "teal")
ppcCand10 <- visualPPC(yrepM = ypredCand0[[1]])
pdf(file.path(PPath, "Results/ppcCand1W0.pdf"))
ppcCand10
dev.off()

pppv_ypredCandW0M1 <- two.sided.pppv(y.obs = pisa18$attp[pisa18$WITHOUT==0], yrepM = ypredCand0[[1]], alternative = "two.sided")

color_scheme_set(scheme = "green")
ppcCand20 <- visualPPC(yrepM = ypredCand0[[2]])
pdf(file.path(PPath, "Results/ppcCand2W0.pdf"))
ppcCand20
dev.off()

pppv_ypredCandW0M2 <- two.sided.pppv(y.obs = pisa18$attp[pisa18$WITHOUT==0], yrepM = ypredCand0[[2]], alternative = "two.sided")

color_scheme_set(scheme = "brightblue")
ppcCand11 <- visualPPC(yrepM = ypredCand1[[1]], WITHOUT = 1)
pdf(file.path(PPath, "Results/ppcCand1W1.pdf"))
ppcCand11
dev.off()

pppv_ypredCandW1M1 <- two.sided.pppv(y.obs = pisa18$attp[pisa18$WITHOUT==1], yrepM = ypredCand1[[1]], alternative = "two.sided", WITHOUT = 1)

color_scheme_set(scheme = "blue")
ppcCand21 <- visualPPC(yrepM = ypredCand1[[2]], WITHOUT = 1)
pdf(file.path(PPath, "Results/ppcCand2W1.pdf"))
ppcCand21
dev.off()

pppv_ypredCandW1M2 <- two.sided.pppv(y.obs = pisa18$attp[pisa18$WITHOUT==1], yrepM = ypredCand1[[2]], alternative = "two.sided", WITHOUT = 1)

save.image(file = file.path(PPath,"Files/save_session.RData"))
#load(file=file.path(cnt.path,"save_session.RData"))

num<-11
sink(file = file.path(PPath,"Results/candW0M1.txt"))
for(i in 1:num){
  print(paste("Table   ",i))
  print(kable(pppv_ypredCandW0M1[[i]], format = "simple", digits = 2))
  print("                 ")
}
sink(file=NULL)


sink(file = file.path(PPath,"Results/candW0M2.txt"))
for(i in 1:num){
  print(paste("Table   ",i))
  print(kable(pppv_ypredCandW0M2[[i]], format = "simple", digits = 2))
  print("                 ")
}
sink(file=NULL)


sink(file = file.path(PPath,"Results/candW1M1.txt"))
for(i in 1:num){
  print(paste("Table   ",i))
  print(kable(pppv_ypredCandW1M1[[i]], format = "simple", digits = 2))
  print("                 ")
}
sink(file=NULL)


sink(file = file.path(PPath,"Results/candW1M2.txt"))
for(i in 1:num){
  print(paste("Table   ",i))
  print(kable(pppv_ypredCandW1M2[[i]], format = "simple", digits = 2))
  print("                 ")
}
sink(file=NULL)
...
pdf(file.path(PPath, "Result/ppcStack0.pdf"))
ppcStack0
dev.off()
...
pdf(file.path(PPath, "Result/ppcStack1.pdf"))
ppcStack1
dev.off()
```
...
pisa18_W0$DP_QU <- as.factor(cut_number(pisa18_W0$FEMFRAC,4))
levels_DP_W0 <- levels(pisa18_W0$DP_QU)
ds_DP_W0 <- pisa18_W0 %>% group_by(DP_QU) %>% summarise(median = median(FEMFRAC), Freq = n())
...
pint0 <- comp.int.stack.core01(sgr = sgr,fits = fits0,
                                     wts = wtsStacking0,
                                     cnt.path = file.path(PPath,"File"), p = 0.5)
```
