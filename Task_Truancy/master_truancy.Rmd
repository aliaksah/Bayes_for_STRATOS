...
###### not of focal interest
```{r  eval = FALSE}
pdf(file.path(PPath, "Results/result0.pdf"))
plot_predictions(fits.imp0[[1]], condition = c("goal","bull","ld"))
plot_predictions(fits.imp0[[1]], condition = c("goal","native","ld"))
plot_predictions(fits.imp0[[1]], condition = c("goal","bull","native"))
dev.off()
```
###### not of focal interest
```{r eval = FALSE}
pdf(file.path(PPath, "Results/result1.pdf"))
plot_predictions(fits.imp1[[1]], condition = c("goal","bull","ld"))
plot_predictions(fits.imp1[[1]], condition = c("goal","native","ld"))
plot_predictions(fits.imp1[[1]], condition = c("goal","bull","native"))
dev.off()
```

##### result of focal interest, WITHOUT = 0
```{r eval = FALSE}
conditional_preds0 <- predictions(
  fits.imp0[[1]], 
...
```{r eval = FALSE}
conditional_preds1 <- predictions(
  fits.imp1[[1]], 
  newdata = datagrid(ld = c(0, 1)), 
  by = "ld", 
  re_formula = NA
